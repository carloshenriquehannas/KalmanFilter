...
F = np.zeros((6, 6))                #Inicia a matriz F e a preenche com os respectivos valores
for i in range(0, 6):
    F[i, i] = 1                     #Preenche a diagonal principal
F[0, 1] = F[1, 2] = F[3, 4] = F[4, 5] = tempo
F[0, 2] = F[3, 5] = 0.5 * (tempo ** 2)

Q = np.zeros((6, 6))                #Inicia a matriz Q e a preenche com os respectivos valores
...
R = np.zeros((2, 2))                #Inicia a matriz R e a preenche com os respectivos valores
R[0, 0] = R[1, 1] = sigma_medicao**2

P = np.zeros((6, 6))                #Inicia a matriz P e a preenche com valores arbitrarios
for i in range(0, 6):
    P[i, i] = 500                   #Preenche a diagonal principal

H = np.zeros((2, 6))                #Inicia a matriz H e a preenche com valores arbitrarios
H[0, 0] = H[1, 3] = 1

I = np.zeros((6, 6))                  #Inicia a matriz I como identidade
for i in range(0, 6):
    I[i, i] = 1
